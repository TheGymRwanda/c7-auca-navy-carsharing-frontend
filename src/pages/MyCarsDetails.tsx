...
export default function MyCarsDetails() {
  ...
  useEffect(() => {
    ...
    async function fetchCarDetails() {
      try {
        const carData = (await fetchWithAuth(`/cars/owned/${id}`)) as Car
        if (!mounted) return
        setCar(carData)
  ...
  return (
    <div className="flex min-h-screen flex-col items-center bg-gradient-to-b to-sky-700 text-white">
      ...
      <div className="mt-6 w-full space-y-3 px-10 text-left">
        ...
        <Button title="Delete Car" to={`${AppRoutes.deleteMyCar}/${id}`} variant="filled" className='bg-red-500'/>
      </div>
    </div>
  )
}
